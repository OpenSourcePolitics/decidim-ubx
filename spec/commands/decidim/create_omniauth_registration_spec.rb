  ...
  module Comments
    describe CreateOmniauthRegistration do
      describe "call" do
        ...
        context "with CreateOmniauthRegistrationExtends" do
          before do
          ...
          it "verifies OAuth signature" do
            expect(command).to receive(:verify_oauth_signature!)
            command.call
          end

          it "broadcasts ok when existing identity is found" do
            user = create(:user, email: email, organization: organization)
            create(:identity, user: user, provider: provider, uid: uid)
          ...
          it "calls trigger_omniauth_registration when existing identity is found" do
            user = create(:user, email: email, organization: organization)
            create(:identity, user: user, provider: provider, uid: uid)
          ...
          it "broadcasts error when record is invalid" do
            allow(command).to receive(:create_or_find_user).and_raise(ActiveRecord::RecordInvalid)
            expect { command.call }.to broadcast(:error)
          end
        end
      end
